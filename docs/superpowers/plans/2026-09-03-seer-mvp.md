...
- Produces:
  ...
  export interface DreamDexExchange {
    ...
    client: { getMarketOnchain(marketId: `0x${string}`): Promise<MarketOnchain> };
  }
  ...
  it("returns null when the read throws", async () => {
    const ctx: DreamDexContext = {
      ...
      exchange: {
        ...
        fetchOrderBook: vi.fn(),
        fetchPrice: vi.fn(),
        client: { getMarketOnchain: vi.fn().mockRejectedValue(new Error("not found")) },
      },
    };
...
**Interfaces:**
- Consumes: `DreamDexContext`, `DreamDexExchange`, `marketOnchain`, `outcomeSymbols` from Task 3. `boundaryPrice`, `getOpeningPrices`, `BinaryMarket` from `@somnia-chain/markets-sdk`.
- Produces:
  ```ts
...
describe("normalizeMarkets", () => {
  it("resolves a fixed-strike market's reference price from strike", async () => {
    const ctx: DreamDexContext = {
      ...
      exchange: {
        ...
        fetchPrice: vi.fn(),
        client: { getMarketOnchain: vi.fn().mockResolvedValue(onchain()) },
      },
    };
  ...
  it("resolves an up/down market's reference price from the posted opening answer", async () => {
    ...
    const [view] = await normalizeMarkets(ctx, [referenceMarket()], {
      // injected opening-price fetcher, see Step 3's optional param
  ...
  it("maps every on-chain MarketStatus number to its label", async () => {
    const ctx: DreamDexContext = {
      ...
      exchange: {
        ...
        fetchOrderBook: vi.fn().mockResolvedValue({ bids: [], asks: [] }),
        fetchPrice: vi.fn(),
        client: { getMarketOnchain: vi.fn().mockResolvedValue(onchain({ status: 4 })) },
      },
    };
    const [view] = await normalizeMarkets(ctx, [fixedStrikeMarket()]);
...
import { boundaryPrice, getOpeningPrices, type BinaryMarket, type UnifiedMarket } from "@somnia-chain/markets-sdk";
...
interface NormalizeDeps {
  /** Injectable for tests; defaults to the real SDK function. */
  fetchOpeningPrices: (ids: string[], indexerUrl: string) => Promise<Record<string, string | null>>;
}

const DEFAULT_DEPS: NormalizeDeps = { fetchOpeningPrices: getOpeningPrices };
...
export async function normalizeMarkets(
  ...
  const openingPrices =
    referenceIds.length > 0 ? await deps.fetchOpeningPrices(referenceIds, ctx.config.indexerUrl) : {};
...
Note: `boundaryPrice`'s `raw` is in "the oracle's price scale," not necessarily human units — verify the actual scale against `docs.dreamdex.io/developers/event-contracts` or a real market row's `strike` vs. its known real-world price at Task 22 (integration check); if a scale divisor is needed, add it here with a named constant and a comment citing where the scale was confirmed, rather than guessing.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const estimate = estimateUp({
      spot: momentum.spot,
      r: momentum.r,
      strike: view.referenceKind === "strike" ? view.referencePrice : null,
      timeToExpiryMs,
      windowMs: 60_000,
      expectedMove: 0.01,
      sensitivity: 2,
      model: view.referenceKind === "strike" ? "strike" : "momentum",
      anchorUp,
    });
